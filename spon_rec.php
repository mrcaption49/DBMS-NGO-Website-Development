...
$results = mysqli_query($conn,"SELECT * FROM SPON_REC");
$edit_state = false;

if(isset($_GET['edit'])){
	$id = $_GET['edit'];
	$edit_state = true;
	$rec = mysqli_query($conn,"SELECT * FROM SPON_REC WHERE id=$id");
...
<div class="header">
<p style="color:#ffffff; font-family:Trebuchet MS; font-size:20px"><span align="left">Create New Activity.</span></p>
</div>
...
<div class="row">
  <div class="col-3 menu">
    <ul>
      ...
      <li class="active" ><a href="#" style="text-decoration:none; color:#ffffff">Sponseres Record</li>
  ...
  <div class="col-9">
    <h1>Sponsers Record</h1>
    <table>
	<thead>
	 <tr>
	  <th>Sponserer Name</th>
	  <th>Sponserer Details</th>
	...
	<?php while ($row = mysqli_fetch_array($results)){ ?>
	 <tr>
	  ...
	  <td>
	   <a style="text-decoration:none; color:#000000" href="spon_rec.php?edit=<?php echo $row['id']; ?>">Edit</a>
	  </td>
	  <td>
	   <a style="text-decoration:none; color:#000000" href="spon_rec.php?del=<?php echo $row['id']; ?>">Delete</a>
	  </td>
	 </tr>
	<?php } ?>
	</tbody>
...
if($name!="" && $detail!="")
{
$query = "INSERT INTO SPON_REC VALUES (NULL, '$name', '$detail')";
mysqli_query($conn,$query);
header('location: spon_rec.php');
echo "<script>window.open('spon_rec.php','_self')</script>";
}
else
{
...
mysqli_query($conn, "UPDATE SPON_REC SET name='$name', details='$detail' WHERE id='$id'");
header('location: spon_rec.php');
echo "<script>window.open('spon_rec.php','_self')</script>";
}
...
if(isset($_GET['del'])){
	$id = $_GET['del'];	
	mysqli_query($conn,"DELETE FROM SPON_REC WHERE id='$id'");
	header('location: spon_rec.php');
	echo "<script>window.open('spon_rec.php','_self')</script>";
}
?>