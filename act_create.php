...
if(isset($_GET['edit'])){
	$id = $_GET['edit'];
	$edit_state = true;
	$rec = mysqli_query($conn,"SELECT * FROM ACT_CR WHERE id='$id'");
...
<div class="row">
  <div class="col-3 menu">
    <ul>
	  ...
	  <li class="active"><a href="#" style="text-decoration:none; color:#ffffff">Create Activity<a></li>
  ...
  <div class="col-9">
	...
	<?php while ($row = mysqli_fetch_array($results)){ ?>
	 <tr>
	  ...
	  <td>
	   <a style="text-decoration:none; color:#000000" href="act_create.php?edit=<?php echo $row['id']; ?>">Edit</a>
	  </td>
	  <td>
	   <a style="text-decoration:none; color:#000000" href="act_create.php?del=<?php echo $row['id']; ?>">Delete</a>
	  </td>
	 </tr>
	<?php } ?>
	</tbody>
...
if($name!="" && $detail!="")
{
$query = "INSERT INTO ACT_CR VALUES (NULL, '$name', '$detail')";
mysqli_query($conn,$query);
header('location: act_create.php');
echo "<script>window.open('act_create.php','_self')</script>";
}
else
{
...
mysqli_query($conn, "UPDATE ACT_CR SET name='$name', detail='$detail' WHERE id='$id'");
header('location: act_create.php');
echo "<script>window.open('act_create.php','_self')</script>";
...
if(isset($_GET['del'])){
	$id = $_GET['del'];	
	mysqli_query($conn,"DELETE FROM ACT_CR WHERE id='$id'");
	header('location: act_create.php');
	echo "<script>window.open('act_create.php','_self')</script>";
}
?>